...
import "./Banner.css";
import { Banner_content } from "./sections/Banner_content";

export const Banner = () => {
  const {
    data: movieList,
    isOk,
  } = useAxios<IMovieLists_Response>(requests.fetchNowPlaying, axios);
  ...
  return (
    <header>
      {isOk && movieList && <Banner_content movieId={movieId} />}
    </header>
  );
};
